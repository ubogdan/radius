...
func (p *Packet) Encode() ([]byte, error) {
	...
	// The Message-Authenticator MUST be used for every packet, which includes
	// the Extensible Authentication Protocol (EAP) message (RFC 3579).
	if _, isEAPMessage := p.Lookup(EAPMessage); isEAPMessage {
		// Clear any signature and add fresh empty one
		p.Del(MessageAuthenticator)
		p.Add(MessageAuthenticator, make([]byte, 16))
	}
	...
	if _, isEAPMessage := p.Lookup(EAPMessage); isEAPMessage {
		hash := hmac.New(crypto.MD5.New, []byte(p.Secret))
		hash.Write(b)
		p.Set(MessageAuthenticator, hash.Sum(nil))
		p.Attributes.encodeTo(b[20:])
	}
